...
3. Operate on an existing artifact: inspect `/workspace/output`, resolve the source file, then use normal shell/code/filesystem tools.
4. Continue prior work: use prior conversation context plus `/workspace/output` state.
...
Use the `gai` package only for specialized new media generation. Use the mounted hard path from the `gemini-anything` skill, run `bash "$GAI" --help`, then run the relevant subcommand help before generation. Never use a bare `gai` executable, create a wrapper, run helper scripts, inspect npm-cache files, or search the whole filesystem to locate it.

Do not use `gai` to transform existing files. For conversions, trimming, resizing, packaging, renaming, or moving files, use normal shell/code tools such as `ffmpeg`.
