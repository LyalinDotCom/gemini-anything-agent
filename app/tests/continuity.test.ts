...
describe("conversation continuity", () => {
  it("only continues from completed successful turns", () => {
    ...
    expect(latestContinuableSession([complete, running, failed, serverFailed], "gemini-anything-agent")?.localId)
      .toBe("complete");
    expect(latestReusableEnvironmentSession([complete, running, failed, serverFailed], "gemini-anything-agent")?.localId)
      .toBe("complete");
  });
